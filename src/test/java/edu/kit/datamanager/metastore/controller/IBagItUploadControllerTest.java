...
public class IBagItUploadControllerTest {
  ...
  public void testHandleFileUploadWithPathToMets() throws Exception {
    ...
    this.mockMvc.perform(get("/api/v1/metastore/bagit").accept(MediaType.TEXT_HTML))
            .andDo(print())
            .andExpect(status().isOk())
            .andExpect(MockMvcResultMatchers.xpath("/html/body/div/table/tbody/tr/td/a[@href = '" + redirectedUrl + "']").exists())
            .andReturn();
    ...
    this.mockMvc.perform(get("/api/v1/metastore/bagit").accept(MediaType.TEXT_HTML))
            ...
            .andExpect(MockMvcResultMatchers.xpath("/html/body/div/table/tbody/tr/td/a[@href = '" + redirectedUrl2 + "']").exists())
            .andReturn();
  }
  ...
  @Test
  @WithMockUser(username = "ingest", password = "forTestOnly", roles = "USER")
  public void testHandleFileUploadWithProvenanceAndTextRegions() throws Exception {
    System.out.println("*****************************************************");
    System.out.println("testHandleFileUploadWithProvenanceAndTextRegions");
    String filename = "validCompleteBag.ocrd.zip";
    String ocrdIdentifier = "anyIdentifier";
    File ocrdZip = new File("/tmp/" + filename);
    if (ocrdZip.exists()) {
      ocrdZip.delete();
    }
    File testDir = Files.createTempDirectory("testUploadCompleteBag_").toFile();
    File metadataDir = new File("src/test/resources/all/metadata");
    File srcDir = new File("src/test/resources/all/data");
    FileUtils.copyDirectory(srcDir, testDir);
    try {
      Bag result = BagItUtil.buildBag(testDir, metadataDir, ocrdIdentifier);
      BagWriter.write(result, testDir.toPath());
      ZipUtils.zip(testDir, ocrdZip);
    ...
    System.out.println("*****************************************************");
    System.out.println("listFilteredUploadedFilesByOcrdIdentifier");
    String[] ocrdIdentifierArray = {ocrdIdentifier};
    this.mockMvc.perform(get("/api/v1/metastore/bagit/ocrdidentifier")
            .param("ocrdidentifier", ocrdIdentifierArray))
            .andDo(print())
            .andExpect(status().isOk())
            .andExpect(MockMvcResultMatchers.jsonPath("$.[*]", Matchers.hasItem(dataObjectId)))
            .andReturn();
